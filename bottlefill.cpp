...
void BottleFill::almacenarBotellas()
{
    ...
    {
        qInfo() << "Aún no hay suficientes botellas para pasarlas a almacen";
    }
    labelBotellasAlmacen->setText("Botellas en Almacen: " + QString::number(botellas_almacen));
    emit setValue(botellas_almacen);
    emit sendALmacen(botellas_almacen,latas_almacen);
}
